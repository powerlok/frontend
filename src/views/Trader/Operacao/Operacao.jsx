...
class Operacao extends React.Component {
    ...
    componentWillMount() {
        if (this.props.history.location.state) {
            ...
            var seqcontacorrentef = this.props.history.location.state.seqcontacorrentef;
            //console.log(de, ate);
            this.setState({ id: (id !== '') ? id : 0, de: de, ate: ate, seqcontacorrentef: seqcontacorrentef });
            this.loadData(id);

            setTimeout(() =>{
                this.getSaldoOperacao();
                /*if((moment(new Date()).format('YYYY-MM-DD') <= moment(this.state.data).format('YYYY-MM-DD')) && this.state.id > 0){
                    this.getSaldoOperacao();
                }*/
            }, 500);
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.trader.traders !== this.props.trader.traders) {
            this.setState({
                ...
                percmeta: (nextProps.trader.traders.percmeta !== 0) ? nextProps.trader.traders.percmeta : '',
                vlrmeta: (nextProps.trader.traders.vlrmeta !== 0) ? nextProps.trader.traders.vlrmeta : '',
                vlrmetaacumulada: (nextProps.trader.traders.vlrmetaacumulada !== 0) ? nextProps.trader.traders.vlrmetaacumulada : '',
                vlrrealizado: (nextProps.trader.traders.vlrrealizado !== 0) ? nextProps.trader.traders.vlrrealizado : '',
                percrealizado: (nextProps.trader.traders.percrealizado !== 0) ? nextProps.trader.traders.percrealizado : '',
                vlracumuladobruto: (nextProps.trader.traders.vlracumuladobruto !== 0) ? nextProps.trader.traders.vlracumuladobruto : '',
                vlrliquido: (nextProps.trader.traders.vlrliquido !== 0) ? nextProps.trader.traders.vlrliquido : '',
                percrealizadoliquido: (nextProps.trader.traders.percrealizadoliquido !== 0) ? nextProps.trader.traders.percrealizadoliquido : '',
                vlracumuladoliquido: (nextProps.trader.traders.vlracumuladoliquido !== 0) ? nextProps.trader.traders.vlracumuladoliquido : '',
                perctaxas: (nextProps.trader.traders.perctaxas !== 0) ? nextProps.trader.traders.perctaxas : '',
                vlrtaxas: (nextProps.trader.traders.vlrtaxas !== 0) ? nextProps.trader.traders.vlrtaxas : '',
                percrealizadoacumuladobruto: (nextProps.trader.traders.percrealizadoacumuladobruto != 0) ? nextProps.trader.traders.percrealizadoacumuladobruto : '',
            });
        }

        if (nextProps.trader !== this.props.trader) {
            this.setState({ saldo_operacao: nextProps.trader.saldo });
            this.setState({ vlrtotalacummeta: nextProps.trader.vlracummeta });
            this.setState({ vlrtotalacumbruto: nextProps.trader.vlracumbruto });
            this.setState({ vlrtotalacumliquido: nextProps.trader.vlracumliquido });
        }
    ...
    handleVoltar() {
        this.props.history.push("/admin/trader/operacao/operacaoabas",{ "de": this.state.de, "ate": this.state.ate, "seqcontacorrentef": this.state.seqcontacorrentef });
    }
    ...
    getSaldoOperacao() {
       ...
       // var percmeta = this.state.percmeta;
        ...
        if ((data !== '' &&  data !== undefined) && (seqcontacorrente !== '' && seqcontacorrente !== undefined)) {     
            ...
            setTimeout(() => { 
                var saldo = Number(this.state.saldo_operacao); //console.log(Number(this.state.saldo_operacao));
    ...
    validate(e) {
        ...
        if (name != undefined) {
            ...
            if (!value || (name == 'saldo_operacao' && (value.length == 0 || value.length == "0,00"))) {
                return this.setState({ [`${name}Error`]: true, [`${name}Message`]: "Campo obrigatório" });
            }

            if (!value || (name == 'percmeta' && (value.length == 0 || value.length == "0,00"))) {
                return this.setState({ [`${name}Error`]: true, [`${name}Message`]: "Campo obrigatório" });
            }
        }
